...
The purpose of this utility app is to convert `.gp` files into a format that is suitable for use in the BassHero ESP32 Game.

## Why `.gp` files?
There are lots of website that host `.gp` files, so it is very supported and easy to find your favorite songs tab. Some websites include:
...
2. **Download a `.gp` file from the web.**
3. **Upload the `.gp` file to the app.**
...
A python script `python_api/main.py` uses the [PyGuitarPro Package](https://github.com/Perlence/PyGuitarPro) to read `.gp` files.
